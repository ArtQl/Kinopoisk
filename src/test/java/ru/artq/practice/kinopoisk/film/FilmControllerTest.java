...
abstract class FilmControllerTest {
    ...
    void test1_addFilm() throws Exception {
        ...
        mockMvc.perform(post("/films")
                ...
                status().isNotFound(),
                result -> assertInstanceOf(FilmIdException.class, result.getResolvedException(), "ID Exist")
        );
        film.setId(1);
        mockMvc.perform(post("/films")
                .content(mapper.writeValueAsString(film))
    ...
    void test2_addFilmWithWrongDuration() throws Exception {
        film = new Film("a", "description value",
                LocalDate.of(2020, 12, 31), Duration.ofMinutes(-21));
        mockMvc.perform(post("/films")
                .content(mapper.writeValueAsString(film))
                .contentType(MediaType.APPLICATION_JSON)
        ).andExpectAll(
    ...
    void test5_updateNotFound() throws Exception {
        ...
        mockMvc.perform(put("/films")
                ...
                status().isNotFound(),
                result -> assertInstanceOf(FilmIdException.class, result.getResolvedException())
        );
    }
